...
public class FactionsDemoteSubCommand implements CommandExecutor {

    private HCF plugin;

    public FactionsDemoteSubCommand(HCF plugin) {
        this.plugin = plugin;
    }

    public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
        if (args.length != 2) {
            sender.sendMessage(ChatColor.RED + "Usage: /" + commandLabel + " demote <player>");
            return true;
        }
        ...
        if (targetMember.getFactionRole().equals(FactionRole.MEMBER)) {
            sender.sendMessage(Messages.ALREADY_MEMBER.getMessage());
            return true;
        }
        targetMember.setFactionRole(FactionRole.MEMBER);
        sender.sendMessage(Messages.PROMOTED_PLAYER.getMessage().replace("%player%", offlinePlayer.getName()).replace("%role%", "Member"));
        if (offlinePlayer.isOnline()) {
            Player target = this.plugin.getServer().getPlayer(offlinePlayer.getUniqueId());
            target.sendMessage(Messages.PROMOTED.getMessage().replace("%role%", "Member"));
        }
        return true;
    }
}
